...
# References:
#   IBM System/2, Keyboards (101- and 102-Key): http://mcamafia.de/pdf/ibm_hitrc11.pdf
#   PS/2 PC Keyboard Scan Sets Translation table: https://www.vetra.com/scancodes.html
ibm_keys = {
    0x29:0x1, 0x2:0x2, 0x3:0x3, 0x4:0x4, 0x5:0x5, 0x6:0x6, 0x7:0x7, 
    0x8:0x8, 0x9:0x9, 0xa:0xa, 0xb:0xb, 0xc:0xc, 0xd:0xd, 0xe:0xf, 
    0xf:0x10, 0x10:0x11, 0x11:0x12, 0x12:0x13, 0x13:0x14, 0x14:0x15, 
    0x15:0x16, 0x16:0x17, 0x17:0x18, 0x18:0x19, 0x19:0x1a, 0x1a:0x1b, 
    0x1b:0x1c, 0x2b:0x1d, 0x3a:0x1e, 0x1e:0x1f, 0x1f:0x20, 0x20:0x21, 
    0x21:0x22, 0x22:0x23, 0x23:0x24, 0x24:0x25, 0x25:0x26, 0x26:0x27, 
    0x27:0x28, 0x28:0x29, 0x1c:0x2b, 0x2a:0x2c, 0x2c:0x2e, 0x2d:0x2f, 
    0x2e:0x30, 0x2f:0x31, 0x30:0x32, 0x31:0x33, 0x32:0x34, 0x33:0x35, 
    0x34:0x36, 0x35:0x37, 0x36:0x39, 0x1d:0x3a, 0x38:0x3c, 0x39:0x3d, 
    0xe038:0x3e, 0xe01d:0x40, 0xe052:0x4b, 0xe04b:0x4f, 0xe047:0x50, 
    0xe04f:0x51, 0xe048:0x53, 0xe050:0x54, 0xe049:0x55, 0xe051:0x56, 
    0xe04d:0x59, 0x45:0x5a, 0x47:0x5b, 0x4b:0x5c, 0x4f:0x5d, 0xe035:0x5f, 
    0x48:0x60, 0x4c:0x61, 0x50:0x62, 0x52:0x63, 0x37:0x64, 0x49:0x65, 
    0x4d:0x66, 0x51:0x67, 0x53:0x68, 0x4a:0x69, 0x4e:0x6a, 0xe01c:0x6c, 
    0x1:0x6e, 0x3b:0x70, 0x3c:0x71, 0x3d:0x72, 0x3e:0x73, 0x3f:0x74, 
    0x40:0x75, 0x41:0x76, 0x42:0x77, 0x43:0x78, 0x44:0x79, 0x57:0x7a, 
    0x58:0x7b, 0xe02a:0x7c, 0x46:0x7d, 0xe11d:0x7e, 0x56:0x2d
} 

# Converts PS/2 Set 1 scan code (8 or 16 bits) to IBM key number (7 bits); Returns 0 if scan code not recognized
def ps2_to_ibmkey(scancode):
    if scancode in ibm_keys:
        return ibm_keys[scancode]
    else:
        return 0x00
...
for iso_mode in [False, True]:
	...
	for hid_scancode in layout.keys():
		ps2_scancode = ps2_to_ibmkey(hid_scancode)
	...
	for shiftstate in keytab.keys():
		...
		keytab[shiftstate][114]   = chr(0x86) # f3
		keytab[shiftstate][115]   = chr(0x8a) # f4
		keytab[shiftstate][116]   = chr(0x87) # f5
		keytab[shiftstate][117]   = chr(0x8b) # f6
		keytab[shiftstate][118]   = chr(0x88) # f7
		keytab[shiftstate][119]   = chr(0x8c) # f8

		# C128 addition
		keytab[shiftstate][110] = chr(0x1b) # Esc

		# C65 additions
		keytab[shiftstate][120]  = chr(0x10) # f9
		keytab[shiftstate][121]  = chr(0x15) # f10
		keytab[shiftstate][122]  = chr(0x16) # f11
		keytab[shiftstate][123]  = chr(0x17) # f12

		# numpad
		keytab[shiftstate][99]  = '0'
		keytab[shiftstate][93]  = '1'
		keytab[shiftstate][98]  = '2'
		keytab[shiftstate][103] = '3'
		keytab[shiftstate][92]  = '4'
		keytab[shiftstate][97]  = '5'
		keytab[shiftstate][102] = '6'
		keytab[shiftstate][91]  = '7'
		keytab[shiftstate][96]  = '8'
		keytab[shiftstate][101] = '9'
		keytab[shiftstate][100] = '*'
	...
	petscii_from_ctrl_scancode = [ # Ctrl
		...
		(8, 0x1f),    # '7'
		(9, 0x9e),    # '8'
		(10, 0x12),   # '9' REVERSE ON
		(11, 0x92),   # '0' REVERSE OFF
	]
	...
	for shiftstate in keytab.keys():
		keytab[shiftstate][83] 	= chr(0x91) # Up arrow
		keytab[shiftstate][84] 	= chr(0x11) # Down arrow
		keytab[shiftstate][79] 	= chr(0x9d) # Left arrow
